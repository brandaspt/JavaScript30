...
console.log(inventors.reduce((prev, curr) => prev + (curr.passed - curr.year), 0))
...
// const filtered = names.filter(name => name?.includes('de'))

// 7. sort Exercise
console.log("Sort the people alphabetically by last name")
console.log(
	people.sort((a, b) => {
		const aSurname = a.split(" ")[1].toLowerCase()
		const bSurname = b.split(" ")[1].toLowerCase()
		if (aSurname < bSurname) return -1
		if (bSurname < aSurname) return 1
		return 0
	})
)
...
const instancesObj = data.reduce((acc: any, curr) => {
